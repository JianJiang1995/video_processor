...
def create_window_summary(
    ...
    # Real-time updates are latency-sensitive and arrive frequently. Do not let
    # summary compression compete with the live analysis/VLM queue; clinical
    # reports are generated by a separate offline endpoint.
    stage = (others_data or {}).get("stage") if isinstance(others_data, dict) else None
    realtime = bool((others_data or {}).get("realtime")) if isinstance(others_data, dict) else False
    if stage != 1 and not realtime:
...
def get_summaries_by_session(db, session_id) -> List[Dict[str, Any]]:
    ...
    session_str = _resolve_session_id(session_id)
    summaries = get_mysql_service().get_all_window_summaries(session_str)
    return [
        {
            **item,
            "session_id": session_str,
            "timestamp": item.get("window_start"),
            "analysis_type": "window",
        }
