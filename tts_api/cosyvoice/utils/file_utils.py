...
def load_wav(wav, target_sr, min_sr=16000):
    ...
    # Use librosa to load. librosa supports file paths and file-like objects (via audioread/soundfile)
    if hasattr(wav, 'read'): # file-like object
        wav.seek(0)
    
    # librosa.load returns (y, sr), y is numpy array (channels, samples) or (samples,) if mono
    # we force mono=True to match original mean(dim=0)
    speech, sample_rate = librosa.load(wav, sr=None, mono=True)
    ...
    # Convert back to torch tensor: (1, T)
